...
"3.1.1" match {
  case ScalaVersion(major,minor, _, PreRelease(preV))  => s"pre-release: $major.$minor - $preV"
  case ScalaVersion(IsScala3(), _,_, _) => "Scala 3 release"
  case s"2.${minor}.${patch}" => s"Scala 2.$minor release"
  case Parts(major, minor, other @ _ *) => s"other sem ver version $major.$minor - ${other.mkString("/")}"
...
object Parts{
  def unapplySeq(s: String): Option[Seq[String]] = Some(s.split('.').flatMap(_.split('-')).toList)
}
